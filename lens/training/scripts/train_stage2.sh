#!/usr/bin/env bash
# LENS — Stage 2: supervised fine-tuning (full-parameter, Qwen2.5-14B / 7B + TS encoder).
#
# Requires GPUs + the ChatTS-Training submodule checked out under
# third_party/ChatTS-Training (see third_party/README.md and
# lens/training/README.md). NOT run by CI.
#
# Adapted from ChatTS-Training/scripts/train_chatts_stage2.sh — all absolute
...
#   LENS_CHATTS_DIR    ChatTS-Training submodule (default: $LENS_REPO_ROOT/third_party/ChatTS-Training)
#   LENS_MODEL_DIR     base model path (default: $LENS_OUTPUT_DIR/lens-stage1, the Stage-1 checkpoint)
...
#   LENS_DEEPSPEED     DeepSpeed config (default: $LENS_REPO_ROOT/configs/training/deepspeed/ds_config_3.json)
#   LENS_NUM_GPUS      GPUs per node                      (default: 8)
#   LENS_MASTER_PORT   deepspeed launcher port            (default: 19901)
#   LENS_STAGE_YAML    training YAML template             (default: $LENS_REPO_ROOT/configs/training/stage2.yaml)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LENS_REPO_ROOT="${LENS_REPO_ROOT:-$(cd "$SCRIPT_DIR/../../.." && pwd)}"

LENS_CHATTS_DIR="${LENS_CHATTS_DIR:-$LENS_REPO_ROOT/third_party/ChatTS-Training}"
LENS_OUTPUT_DIR="${LENS_OUTPUT_DIR:-$LENS_REPO_ROOT/models}"
LENS_MODEL_DIR="${LENS_MODEL_DIR:-$LENS_OUTPUT_DIR/lens-stage1}"
LENS_DATA_DIR="${LENS_DATA_DIR:-data}"
LENS_DEEPSPEED="${LENS_DEEPSPEED:-$LENS_REPO_ROOT/configs/training/deepspeed/ds_config_3.json}"
LENS_NUM_GPUS="${LENS_NUM_GPUS:-8}"
LENS_MASTER_PORT="${LENS_MASTER_PORT:-19901}"
LENS_STAGE_YAML="${LENS_STAGE_YAML:-$LENS_REPO_ROOT/configs/training/stage2.yaml}"
...
if [[ ! -d "$LENS_CHATTS_DIR/src" ]]; then
  echo "error: ChatTS-Training submodule not found at $LENS_CHATTS_DIR" >&2
  echo "       run: git submodule update --init --recursive third_party/ChatTS-Training" >&2
  exit 1
fi

RENDERED_YAML="$(mktemp -t lens_stage2_XXXXXX.yaml)"
