...
`chatts` template + TS encoder), included as a git submodule at
`./ChatTS-Training` (pinned at `cf2139a8`). Needs GPUs + that submodule — not
run by CI.
...
lens/training/dataset_info.template.json   dataset registration to copy into the ChatTS-Training submodule's data/
```

## 1. Get the ChatTS-Training submodule

```bash
git submodule update --init ChatTS-Training      # (or clone the repo with --recursive)
# it is pinned at cf2139a8; install per ChatTS-Training's own docs:
pip install -e ./ChatTS-Training                  # or follow that repo's install instructions
```
...
Register them in the ChatTS-Training submodule's `data/`:

```bash
cp lens/training/dataset_info.template.json ChatTS-Training/data/dataset_info.json   # or merge entries
# then symlink/copy your LENS jsonl dirs into ChatTS-Training/data/, e.g.
ln -s "$PWD/data/fake/narrative_dataset" ChatTS-Training/data/narrative_dataset
ln -s "$PWD/data/fake/qa_dataset"        ChatTS-Training/data/qa_dataset
# align_256 / align_random / ift need no local files — they pull from the HF Hub.
```

## 3. Set env vars and run

The launch scripts auto-detect the repo root and default `$LENS_CHATTS_DIR` to
`$LENS_REPO_ROOT/ChatTS-Training` (override it if you cloned the submodule
elsewhere). Override other paths via env vars (see the header of each script).
Minimum you usually set:
...
Other env vars: `LENS_CHATTS_DIR` (default `$LENS_REPO_ROOT/ChatTS-Training`),
`LENS_DATA_DIR` (default `data`, relative to `$LENS_CHATTS_DIR`),
`LENS_DEEPSPEED`, `LENS_MASTER_PORT`, `LENS_STAGE_YAML`. For memory-tight nodes
point `LENS_DEEPSPEED` at the `*_offload.json` variants.
