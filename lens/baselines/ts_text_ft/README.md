...
Upstream [LLaMA-Factory](https://github.com/hiyouga/LLaMA-Factory) is included as
a git submodule at `./LLaMA-Factory`:

```bash
git submodule update --init LLaMA-Factory       # (or git clone the repo with --recursive)
pip install -e "./LLaMA-Factory[torch,metrics]"
```

(ChatTS-Training is itself a LLaMA-Factory fork, so you can alternatively run the
text-FT baseline against the `./ChatTS-Training` submodule.)
